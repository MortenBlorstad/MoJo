...
class Universe():
    ...
    def get_reward(self,state:State, unexplored_count: int) -> float:
        ...
        not_relic_found = ~np.any(state.relic_nodes == 1)
        ...
        reward = np.expand_dims(points_ratio + 0.2*this_points_ratio + point_factor*self.thiscore - unit_score*0.01 - unexplored_ratio * 0.01 -not_relic_found, axis=0) + stacking_in_pointzone_penalty
        return reward
