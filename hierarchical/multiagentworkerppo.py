...
from hierarchical.multiagentutils import Critic, WrkrRolloutBuffer as RolloutBuffer
...
class MultiAgentWorkerPPO:
    ...
    def update(self, worker_id):
        ...
        rewards = []
        discounted_reward = 0
        for reward, is_terminal in zip(reversed(self.bufferList[worker_id].rewards), reversed(self.bufferList[worker_id].is_terminals)):
            if is_terminal:
                discounted_reward = 0
        ...
        rewards = (rewards - rewards.mean()) / (torch.std(rewards, unbiased=False) + 1e-7)
